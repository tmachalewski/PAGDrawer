...
- **Training monitoring** — TensorBoard (native PyTorch `SummaryWriter`, no TensorFlow dependency). The `ml` container writes to a shared `runs/` volume; TensorBoard runs as a docker-compose sidecar on port 6006 (`Scripts/start-tensorboard.sh` / `kill-tensorboard.sh`). Log per epoch: train/val loss, Spearman, CWE/CPE embedding histograms; the embedding projector comes free. **MLflow** noted as a future consolidation option: its tracking + model registry could replace both TensorBoard and the GridFS artifact store with one tool — revisit if/when the GridFS registry starts to chafe. W&B rejected for PoC (hosted; data leaves the machine).
...
- **Stage GML-2 — ML service scaffold.** `ml/` container, docker-compose entry, Scripts, `/train` + `/predict` + `/model/info`, GridFS artifacts (with the label-snapshot file hash). TensorBoard sidecar wired (shared `runs/` volume, port 6006, start/kill scripts). Rung-1/2 models served (multi-hop GNN not needed to ship the service). *Acceptance: end-to-end train→persist→predict cycle through the API; a training run visible live in TensorBoard.*
